...
if ( unpublishedProductsMustBeHidden() ) {
	$filter[] = [
		"LOGIC" => "OR",
		[ "PROPERTY_PUBLISH" => false ],
		[ "PROPERTY_PUBLISH_RETAIL" => false ]
	];
}
else {
	$filter["PROPERTY_PUBLISH_RETAIL"] = false;
}


$arResult["EXCLUDE_ITEMS"] = $APPLICATION->IncludeComponent(
	"gipermed:get.item.ids",
	"",
	array(
		"FILTER" => $filter,
		"CACHE_TIME" => 3600,
...
if (!empty($arResult["ELEMENTS"]) && CModule::IncludeModule("iblock"))
{
	...
	while($arElement = $rsElements->Fetch())
	{
		...
		$arElement["PRICE"] = \CCurrencyLang::CurrencyFormat( $arElement["PROPERTY_PRICE_WHOLESALE_VALUE"], "RUB" );
