...
$rs = CIBlockSection::GetList(
	...
	false,
	array( "ID", "IBLOCK_ID", "IBLOCK_SECTION_ID" , "NAME", 'UF_TEXT_CATEGORY', 'SECTION_PAGE_URL', 'PICTURE' )
);
	...
	foreach($productSections as $sectionId) {
		while ( true ) {
			...
			if ( $parent != ROOT_SECTION_ID ) {
				...
				// if ( (int)$sortIndex > 0 ) {
					$arSections[ $sectionId ]["SRC"] = CFile::GetPath($arSections[ $sectionId ]['PICTURE']);
